...
const advisoryLockName = "social-network-groups-migrations"
...
func applyOne(ctx context.Context, connection *sql.Conn, definition Definition) error {
	...
	case err == nil:
		if checksum != definition.Checksum {
			return fmt.Errorf("Groups migration %s checksum changed after application", definition.Version)
		}
	...
	if _, err := transaction.ExecContext(ctx, definition.SQL); err != nil {
		return fmt.Errorf("apply Groups migration %s: %w", definition.Version, err)
	}
